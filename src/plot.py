...
def plotting_event_orientation(df_row):

    """
    Generates and saves a detailed multi-panel figure summarizing the event orientation analysis for a single seismic event.

    This function processes seismic waveform data and associated metadata from a provided DataFrame row, and creates a comprehensive figure
    displaying rotated seismograms (radial, transverse, and vertical components), AIC curve for phase picking, orientation optimization
    curves, and geographic context maps. The figure includes:
    
    - Rotated seismograms (HHR, HHT, HHZ) with signal and noise windows marked.
    - AIC curve indicating the best changepoint for arrival picking.
    - Azimuthal orientation search results for maximizing transverse signal strength, cross-correlation, and minimizing energy ratios.
    - A global map showing event and station locations.
    - A beachball plot of the event’s focal mechanism colored by depth.
    - A polar plot showing theoretical ray paths (P, PKP, PKIKP) computed from a standard Earth model.

    The function automatically saves the resulting figure as a PNG file in an organized folder structure based on network and station names.

    Parameters
    ----------
    df_row : pandas Series
        A Series (second element of a DataFrame row) containing preprocessed seismic event data, waveform time series, 
        quality metrics, moment tensor solutions, and auxiliary information required for plotting.

    Notes
    -----
    - Relies on predefined constants such as `TIME_FINAL_P`, `TIME_WINDOW`, and `ORIENTATION_OUTPUT`.
    - Assumes waveform arrays (`tr1_data`, `tr2_data`, `trZ_data`) and timing arrays (`trZ_time`) are already populated in the input.
    - Overwrites any existing figure with the same event name, station, and quality flag if present.
    """
...
# -------------------
    ...
    """
    This script generates and saves orientation analysis plots for each station listed in `STATION_LST`.

    For each station:
    - Filters the input `station_df` to obtain data associated with the station.
    - Creates a monthly time range (`PeriodIndex`) based on the station's available data.
    - Builds a figure with two vertically aligned subplots:
    - The top subplot shows the orientation angles (`theta`) as a scatter plot, distinguishing between "good" and "bad" quality measurements.
    - The bottom subplot displays the count of "good" and "bad" events per month as bar plots.
    - Data points are sized according to their signal-to-noise ratio (SNR).
    - Circular statistics (mean and standard deviation) are optionally computed for good-quality measurements, and a boxplot is overlaid.
    - Visual adjustments are applied to the axes, labels, and grids to enhance readability.
    - The final figure is saved in the designated output folder as a `.png` file, with the filename corresponding to the station.
    """